            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                  <table class="table data-table table-hover mb-0">
                      ...
                      <?php
                        $num = 1;
                        $query ="SELECT * FROM tbl_stock_in"; 
                        ...
                        if(mysqli_num_rows($query_run) > 0)
                        {
                            ...
                            { 
                              ...
                              <tr class="white-space-no-wrap" data-toggle="modal" data-target="#viewstockin" onclick="dataofTable(this)">
                                  ...
                                  <td class="tddate"><?=$row['sin_date'];?></td>
                                  <td hidden class="tdcode"><?=$row['sin_codepo'];?></td>
                                  <?php
                                  $tbl_item = $row['sin_itemid'];
                                  $query ="SELECT item_name, item_categoryid FROM tbl_item WHERE item_id = $tbl_item AND item_status = '1'"; 
                                    $query_run = mysqli_query($conn, $query);
                                  ...
                                  <?php
                                    ...
                                    $query ="SELECT us_username FROM tbl_users WHERE us_id  = $us_id AND us_status = '1'"; 
                                      ...
                                      if(mysqli_num_rows($query_run) > 0)
                                      {
                                        ...
                                        { 
                                          ?>
                                            <td class="tdbayer"><?=$rowu['us_username'];?></td>
                                          <?php
                                        }
                                      }
                                    ?>
                                    <?php
                                    $location_id = $row['sin_locationid'];
                                    $query ="SELECT location_name FROM tbl_location WHERE location_id = $location_id AND location_type = '0' AND location_status = '1'"; 
                                      $query_run = mysqli_query($conn, $query);
                                      ...
                                      if(mysqli_num_rows($query_run) > 0)
                                      {
                                        ...
                                        { 
                                          ?>
                                            <td class="tdlocation"><?=$rowl['location_name'];?></td>
                                          <?php
                                        }
                                      }
                                    ?>
                                  <td class="tdqty"><?=$row['sin_quantity'];?></td>
                                  <td hidden class="tdamount"><?=$row['sin_amount'];?></td>
                                  <td class="tdremark text-left"><?=$row['sin_remarks'];?></td>
                              </tr>
