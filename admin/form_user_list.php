        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="d-flex flex-wrap align-items-center justify-content-between mb-4">
                    ...
                    <a href="form_user_add.php" id="btnAddUser" class="btn btn-primary add-list">
                    ...
                    <table class="data-table table mb-0 tbl-server-info" style="background-color: White; border-radius: 10px;">
                        ...
                        <?php
                            ...
                            if(mysqli_num_rows($query_run) > 0)
                            {
                                ...
                                {
                                    $usfId = $row['us_id'];
                                    if($row['us_type'] > '0' && $row['us_status'] > 0){
                                ?>
                                <tr>
                                    <?php
                                        ...
                                        ?>
                                            <td>
                                            <img src="../assets/images/user/<?= $user_profile['usf_image']; ?>" class="img-fluid rounded avatar-50 mr-3" alt="image">
                                            </td>
                                           <td> <?= $user_profile['usf_firstname'] . " " . $user_profile['usf_lastname']; ?> </td>
                                           <td> <?php if($user_profile['usf_gender'] == '1'){ echo "Male"; }else{ echo "Female"; } ?></td>
                                           <td> <?= $user_profile['usf_phone']; ?></td>
                                           <td> <?= $row['us_email']; ?></td>
                                           <td> <?php if($row['us_type'] == "1"){ echo "Admin"; }else{ echo "User"; } ?></td>
                                           <td>
                                            <div class="card-header-toolbar d-flex align-items-center" data-toggle="modal" data-target="#exampleModal">
                                                    ...
                                                    <div class="dropdown-menu fixed-element" aria-labelledby="dropdownMenuButton001" style="position: fixed; will-change: transform; top: 0px; left: 0px; transform: translate3d(-40px, 36px, 0px);" x-placement="bottom-end">
                                                        <a class="dropdown-item" href="form_user_profile.php?us_id=<?=$row['us_id'];?>&us_name=<?=$row['us_username'];?>&us_email=<?=$row['us_email'];?>&us_role=<?=$row['us_type'];?>&us_create_date=<?=$row['us_created_date'];?>" style="justify-content: flex-end;  " >
                                                        ...
                                                        <a class="dropdown-item" href="form_user_update.php?usf_id=<?=$row['us_id'];?>&us_firstName=<?=$user_profile['usf_firstname'];?>&us_lastName=<?=$user_profile['usf_lastname'];?>&us_phone=<?=$user_profile['usf_phone'];?>&us_gender=<?=$user_profile['usf_gender'];?>&us_dob=<?=$user_profile['usf_dob'];?>&us_role=<?=$row['us_type'];?>&us_address=<?=$user_profile['usf_address'];?>&us_image=<?=$user_profile['usf_image'];?>">
                                                        ...
                                                        <a class="dropdown-item" data-toggle="modal" data-target="#exampleModal" onclick="autoClickButton('<?=$row['us_id'];?>','<?=$row['us_username'];?>' )">
                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"  width="20" stroke-width="2" stroke="currentColor" class="svg-icon mr-0 text-secondary">
...
<script>
  ...
  function autoClickButton(usId, usName) {
    document.getElementById('autoButtonClick').click();
    document.getElementById('us_id').value = usId; 
